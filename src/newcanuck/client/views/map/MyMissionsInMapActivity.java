...
public class MyMissionsInMapActivity extends AbstractMapActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		missions = myMissionDatabaseDatasource.getAllMyMissions();
		...
		for (Mission mission : missions) {
			...
			if (mission.getState() == 1) {
				...
				.position(pos)
				.icon(BitmapDescriptorFactory
						.fromResource(R.drawable.finish_small))
				.title(mission.getName()));
			}
		...
		map.setOnInfoWindowClickListener(new OnInfoWindowClickListener() {
			...
			public void onInfoWindowClick(Marker marker) {
				...
				if (index != -1) { // means find it as a mission
					...
					Intent i = new Intent(MyMissionsInMapActivity.this, MissionPageActivity.class);
					i.putExtras(b);
	...
	private void initMapAndGPS() {
		...
		if (!gpsManager.initialize()) {
			Toast.makeText(MyMissionsInMapActivity.this,
